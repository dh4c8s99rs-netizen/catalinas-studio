...
const [customTaskText, setCustomTaskText] = useState(null);
  ...
  return (
    <div className="min-h-screen w-full bg-[#a1dae1] text-slate-800 font-sans flex flex-col items-center justify-between p-4 selection:bg-amber-200 overflow-x-hidden relative">
      {isToDoOpen && (
      ...
      <div className="relative w-full max-w-[420px] flex flex-col mb-16">
        <div className="absolute z-40 w-8 h-8 pointer-events-none animate-[mothFlight_30s_linear_infinite]"><img src={MothA} alt="Moth" className="w-full h-full object-contain animate-[mothFlap_30s_linear_infinite]" /></div>
        <div className="w-full flex flex-col z-10">
          <div className="w-full relative flex items-end">
            <img src={RoofSVG} alt="Studio Roof" className="w-full h-auto block" />
            <div onClick={triggerPigeonFlight} className={`absolute bottom-[37%] w-8 h-8 z-30 select-none cursor-pointer hover:scale-110 ${pigeonFlying ? 'animate-[pigeonFlyAway_4s_ease-in-out_forwards]' : 'animate-[pigeonWalk_14s_infinite]'}`}><img src={PigeonSVG} alt="Pigeon Vector" className="w-full h-full object-contain" /></div>
          </div>
          <div className="w-full relative flex items-end overflow-hidden select-none">
            ...
            {currentStatus === 'painting' && (
              <div className="absolute bottom-[5%] left-[43%] w-20 z-20 pointer-events-none animate-[bob_3s_ease-in-out_infinite] drop-shadow-md">
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-white border border-amber-200 text-amber-700 text-[10px] font-bold px-3 py-1 rounded-full shadow-md whitespace-nowrap">
                  {customTaskText}<div className="w-2 h-2 bg-white border-b border-r border-amber-200 transform rotate-45 absolute -bottom-1 left-1/2 -translate-x-1/2"></div>
          ...
          <div className="w-full relative flex items-end select-none">
            ...
            <div className="absolute bottom-[10%] left-[15.5%] w-8 z-40 cursor-pointer drop-shadow-sm" onMouseEnter={() => setHoverCup(true)} onMouseLeave={() => setHoverCup(false)}>
              <a href="https://buymeacoffee.com/catalinawilliams" target="_blank" rel="noopener noreferrer" className="relative block w-full h-full">
                <div className={`absolute -top-8 left-1/2 -translate-x-1/2 bg-white border border-amber-200 text-amber-700 text-[10px] font-bold px-2 py-0.5 rounded-full shadow-md whitespace-nowrap transition-all duration-200 pointer-events-none ${hoverCup ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>Buy Cata a coffee</div>
          ...
          <div className="w-full relative flex items-end bg-transparent z-40">
            ...
            <div className="absolute bottom-[2%] left-4 z-50 flex flex-col items-center">
              <div className={`transition-all duration-300 ${currentStatus === 'out' ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'} mb-1`}>
                <div className="relative bg-white border border-amber-200 text-amber-700 text-[10px] font-bold px-3 py-1 rounded-full shadow-md whitespace-nowrap z-50">
                  {customTaskText}<div className="w-2 h-2 bg-white border-b border-r border-amber-200 transform rotate-45 absolute -bottom-1 left-1/2 -translate-x-1/2"></div>
                </div>
              </div>
      ...
      <style>{`
        ...
        @keyframes pigeonWalk { 0% { left: 20%; transform: scaleX(1) translateY(0px) rotate(0deg); } 24% { left: 70%; transform: scaleX(1) translateY(0px) rotate(0deg); } 48% { left: 70%; transform: scaleX(1) translateY(0px) rotate(0deg); } 50% { left: 70%; transform: scaleX(-1) translateY(0px) rotate(0deg); } 74% { left: 20%; transform: scaleX(-1) translateY(0px) rotate(0deg); } 98% { left: 20%; transform: scaleX(-1) translateY(0px) rotate(0deg); } 100% { left: 20%; transform: scaleX(1) translateY(0px) rotate(0deg); } }
        @keyframes pigeonFlyAway { 0% { bottom: 15%; transform: scale(1) translateY(0); opacity: 1; } 40% { bottom: 110%; transform: scale(0.4) translateY(-100px); opacity: 0; } 100% { bottom: 15%; transform: scale(1); opacity: 1; } }
        @keyframes mothFlight { 0%, 33.3% { top: 67%; left: 83%; } 40%, 66.6% { top: 40%; left: 9%; } 73.3%, 100% { top: 12%; left: 83.1%; } }
